...
def make_polar_map(
    ...
    if pole == 'North':  # North Hemisphere
        ...
        ax1.set_extent([-180, 180, lat_min, lat_max], cartopy.crs.PlateCarree())

    if pole == 'South':  # South Hemisphere
        ax1 = plt.subplot(111, projection=cartopy.crs.SouthPolarStereo())
        ax1.set_extent([-180, 180, lat_min, lat_max], cartopy.crs.PlateCarree())

    # Compute a circle in axes coordinates, for use as a boundary for the map.
    ...
    try:
        ...
        logger.error('Cartopy was unable add coastlines due to  a '
                     'connection error.')
    # set circle figure panel boundary
...
def get_lat_lims(cfg, cube, pole='North'):
    """
    Return latitude min and max values
    ...
    Returns
    ----------
    (lat_min, lat_max) : tuple
        Latitude minimum and maximum values
    """
    if pole == 'South':  # South Hemisphere
        try:
    ...
    if pole == 'North':  # North Hemisphere
        ...
        try:
            ...
            lat_max = 90.
        return (lat_min, lat_max)
    logger.fatal('get_lat_lims: Not able to determine latitude limits.')
    return False
...
def make_map_plots(
    ...
    for plot_type, plot_time in itertools.product(plot_types, plot_times):
        for layer_index, (layer, cube_layer) in enumerate(cubes.items()):
            ...
            pole = get_pole(cube)
            (lat_min, lat_max) = get_lat_lims(cfg, cube, pole)
