...
then
    echo " "
    echo "Add file/folder group write permission, and fix group name to ns9560k. "
    echo " "
    echo "Usage:"
    echo "  fixperm.sh /path/to/directory/to/be/fixed"
    echo "  e.g.,"
    echo "  fixperm.sh /projects/NS9560K/ESGF/CMIP6"
    echo " "
    echo "** EXIT **"
    exit 1
fi
...
find $1 -user $USER ! -group ns9560k -ls -exec chown $USER:ns9560k {} \; | tee -a $logfile
